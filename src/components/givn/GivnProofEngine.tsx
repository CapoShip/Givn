...
if (typeof process !== "undefined" && process.env?.NODE_ENV !== "production") {
  console.assert(money(0) === "—");
  console.assert(money(500) === "$500");
  console.assert(money(1500) === "$1.5k");

  const t: ProofEntry[] = [
    { id: "a", timestamp: "2024-01-01T00:00Z", amount: 10, ngo: "n", status: "verified" },
    { id: "b", timestamp: "2024-01-02T00:00Z", amount: 0, ngo: "n", status: "missing" },
    { id: "c", timestamp: "2024-01-03T00:00Z", amount: 20, ngo: "n", status: "verified" },
  ];
  console.assert(sumVerified(t) === 30);
  console.assert(mostRecentVerifiedDate(t) === "2024-01-01");
}
  ...
  return (
    <div className="min-h-screen bg-black text-white font-mono">
      ...
      <section className="min-h-screen flex flex-col items-center justify-center text-center px-6 relative">
        ...
        <div className="relative z-10 mt-10 flex gap-3 flex-wrap justify-center">
          <a
            href="/how-it-works"
            className="inline-flex items-center gap-2 border border-white/30 px-5 py-3 text-xs text-white/70 hover:bg-white/10"
          >
          ...
          <a
            ...
            className="inline-flex items-center gap-2 border border-white/30 px-5 py-3 text-xs text-white/70 hover:bg-white/10"
          >
            view proof page <ShieldCheck className="h-4 w-4" />
          </a>
          <a
            href="/request-access"
            className="inline-flex items-center gap-2 bg-white text-black px-5 py-3 text-xs font-semibold hover:opacity-90"
          >
            request access <Zap className="h-4 w-4" />
          </a>
        </div>
      ...
      <section className={`transition-opacity duration-700 ${reveal ? "opacity-100" : "opacity-0"}`}>
        <div className="max-w-4xl mx-auto px-6 pb-32">
          <div className="mb-12">
            ...
            <p className="mt-3 text-lg text-white/60 max-w-xl">This is not a claim. This is a record.</p>
          </div>
      ...
      <section className="border-t border-white/10 py-28 text-center">
        <div className="max-w-xl mx-auto px-6">
          ...
          >
            ...
            >
              Badge status degrades automatically when proof is missing for the next period.
            </div>
          </button>

          <p className="mt-10 text-white/50 text-lg">
            This is not a promise.
            <br />
            This is time-bound verification.
          </p>

          <div className="mt-14 flex flex-wrap gap-3 justify-center">
            <a
              href="/brand/demo-brand"
              className="inline-flex items-center gap-2 border border-white/30 px-6 py-4 text-xs text-white/70 hover:bg-white/10"
            >
              View proof page <ShieldCheck className="h-4 w-4" />
            </a>
            <a
              href="/request-access"
              className="inline-flex items-center gap-2 bg-white text-black px-6 py-4 text-xs font-semibold hover:opacity-90"
            >
              Request access <Zap className="h-4 w-4" />
            </a>
          </div>
          ...
          {badgeOpen && (
            <div className="mt-14 max-w-2xl mx-auto text-left border border-white/10 p-6">
              ...
              <div className="mt-6 text-xs text-white/40">
                Proof is public. Missing proof is public.
              </div>
            </div>
          )}
